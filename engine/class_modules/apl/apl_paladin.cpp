...
void retribution( player_t* p )
{
  ...
  precombat->add_action( "variable,name=pre_es_wake,op=reset,default=0" );
  ...
  cooldowns->add_action( "execution_sentence,if=(cooldown.avenging_wrath.remains>15|talent.radiant_glory)&(target.time_to_die>10)&(cooldown.wake_of_ashes.remains<gcd|variable.pre_es_wake=1)&(!talent.holy_flames|dot.expurgation.ticking)&(!talent.crusade|buff.avenging_wrath.stack>=7|variable.pre_es_wake=0)" );
  ...
  finishers->add_action( "variable,name=ds_castable,value=(active_enemies>=2|buff.divine_arbiter_verdict.up)&!buff.empyrean_legacy.up&!buff.divine_arbiter_divine_storm.up" );
  ...
  finishers->add_action( "divine_storm,if=variable.ds_castable" );
  finishers->add_action( "templars_verdict" );
  ...
  generators->add_action( "wake_of_ashes,if=(cooldown.avenging_wrath.remains>6|talent.radiant_glory)&(!talent.execution_sentence|cooldown.execution_sentence.remains>4|target.time_to_die<10|variable.pre_es_wake=1)&(!raid_event.adds.exists|raid_event.adds.in>10|raid_event.adds.up)" );
  ...
  generators->add_action( "hammer_of_wrath,if=talent.walk_into_light&cooldown.hammer_of_wrath.charges=2" );
